...
const isAdjacentToSymbol = (grid, regex) => {
  ...
  for (let digit of grid) {
    let match = regex.test(digit);
    // console.log(digit, regex.test(digit));
    if (match) {
      //if match, break loop and return true
      digitMatch = true;
    ...
    if (lineIndex === 0) {
      ...
      capturedNumbers.forEach((numberObj) => {
        ...
        for (let index of indices) {
          ...
          console.log('isAdjacent?',isAdjacentToSymbol(grid, symbolRegex));
          const isAdjacent = isAdjacentToSymbol(grid, symbolRegex);
          if (isAdjacent) {
            // console.log('isAdjacentToSymbol! number:', number);
            total += Number(number);
            console.log('num added:', number, 'total:', total);
            break;
          }
        }
      });
      ...
      capturedNumbers.forEach((numberObj) => {
        ...
        for (let index of indices) {
          ...
          console.log('isAdjacent?',isAdjacentToSymbol(grid, symbolRegex));
          let isAdjacent = isAdjacentToSymbol(grid, symbolRegex);
          if (isAdjacent) {
            total += Number(number);
            console.log('num added:', number, 'total:', total);
            break;
